...
const CreatePaymentModal = ({ onClose, onPaymentCreated }) => {
    ...
    const sanitizePaymentField = (name, value) => {
        ...
        if (name === 'beneficiaryAccount') return sanitizers.digits(value, 20);
    ...
    const handleNext = async (e) => {
        ...
        const feeNum = amountNum * 0.02;
        const totalDebit = amountNum + feeNum;

        setIsCheckingBalance(true);
        const balanceCheck = await validateAmount(totalDebit, payment.currency);
        setIsCheckingBalance(false);

        if (!balanceCheck.ok) {
            if (balanceCheck.error) setSubmitError(balanceCheck.error);
            else setErrors({ amount: `Insufficient balance. Available balance in ${payment.currency} is ${balanceCheck.availableBalance?.toFixed(2)}` });
            return;
        }
    ...
    const handleConfirm = async () => {
        ...
        try {
            await api.post('/payments', {
                ...payment,
                beneficiaryName: payment.beneficiaryName.trim(),
                fees,
                total
            });
            onPaymentCreated();
        } catch (err) {
            setSubmitError(getApiErrorMessage(err, 'Payment failed. Please try again.'));
        } finally {
    ...
    return (
        <div className="modal-overlay">
            <div className="modal-content" style={{ position: 'relative' }}>
                ...
                <h2 style={{ marginTop: '24px', marginBottom: '20px' }}>Create International Payment</h2>
                ...
                {step === 'form' ? (
                    <form onSubmit={handleNext} noValidate>
                        <div className="form-group">
                            ...
                            {errors.beneficiaryName && <span className="error">{errors.beneficiaryName}</span>}
                        </div>
                        <div className="form-group">
                            <label htmlFor="beneficiaryAccount">Beneficiary Account *</label>
                            <input id="beneficiaryAccount" name="beneficiaryAccount" type="text" inputMode="numeric" value={payment.beneficiaryAccount} onChange={handleChange} required />
                            {errors.beneficiaryAccount && <span className="error">{errors.beneficiaryAccount}</span>}
                        </div>
                        <div className="form-group">
                            <label htmlFor="amount">Amount *</label>
                            <input id="amount" type="text" inputMode="decimal" name="amount" value={payment.amount} onChange={handleChange} required />
                            {errors.amount && <span className="error">{errors.amount}</span>}
                        </div>
                        <div className="form-group">
                            <label htmlFor="currency">Currency *</label>
                            <select id="currency" name="currency" value={payment.currency} onChange={handleChange}>
                                <option value="ZAR">ZAR</option><option value="USD">USD</option>
                        ...
                        <div className="form-group">
                            ...
                            <select id="provider" name="provider" value={payment.provider} onChange={handleChange} disabled>
                        ...
                        <div className="form-group">
                            ...
                            <select id="swiftCode" name="swiftCode" value={payment.swiftCode} onChange={handleChange} required>
                                {swiftOptions.map(opt => <option key={opt.value} value={opt.value}>{opt.label}</option>)}
                            </select>
                    ...
                    <div className="review-step">
                        ...
                        <table className="review-table">
                            <tbody>
                                ...
                                <tr><td><strong>Fees (2%):</strong></td><td>{fees} {payment.currency}</td></tr>
                                <tr style={{ fontWeight: 'bold', color: 'red' }}><td><strong>Total to debit:</strong></td><td>{total} {payment.currency}</td></tr>
                            </tbody>
                        </table>
                        <div className="form-actions">
                            <button type="button" onClick={() => setStep('form')} disabled={isSubmitting}>Back</button>
