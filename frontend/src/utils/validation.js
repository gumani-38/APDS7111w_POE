...
export const sanitizers = {
    ...
    username: (value) => value.replace(/[^A-Za-z0-9_]/g, '').slice(0, 30),
    search: (value) => value.replace(/[^A-Za-z0-9 _-]/g, '').slice(0, 50),
    employeeNumber: (value) => value.replace(/[^A-Za-z0-9]/g, '').toUpperCase().slice(0, 9),
    amount: (value) => {
...
export const validateCustomerLogin = ({ username, password }) => {
    const errors = {};
